...
class FunctionSever {
	...
	function post_data_normal($url,$data){ // 模拟提交数据函数
		$curl = curl_init(); // 启动一个CURL会话
		curl_setopt($curl, CURLOPT_URL, $url); // 要访问的地址
		curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, FALSE); // 对认证证书来源的检查
		curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, FALSE); // 从证书中检查SSL加密算法是否存在
	...
	function post_xmldata($url,$xmldata){ // 模拟提交数据函数
	...
	function read_static_cache($cache_name)
	{
		...
		$cache_file_path = 'temp/static_caches/' . $cache_name . '.php';
	...
	function write_static_cache($cache_name, $caches)
	{
		...
		$cache_file_path.= 'static_caches/';
		mkdir($cache_file_path);
		$cache_file_path.= $cache_name . '.php';
		$content = "<?php\r\n";
