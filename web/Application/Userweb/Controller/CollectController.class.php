...
class CollectController extends BaseController{
	...
	public function copy_collect(){
		$collect=M("collect")->where("id={$_REQUEST[id]}")->find();
		unset($collect[id]);
		$collect[status]=0;
		$collect[user_id]=$this->user_id;
		$id=M("collect")->add($collect);
		$extends=M("collect_extend_field")->where("collect_id={$_REQUEST[id]}")->select();
		foreach ($extends as $key => $value) {
			$value[collect_id]=$id;
			unset($value[id]);
			M("collect_extend_field")->add($value);
		}
	...
	public function publish_collect(){
		...
		if($res["errmsg"] != "ok"){
			...
			if($res[errcode]==60011){
				$this->showmsg("管理组权限不足，（user/department/agent）无权限");
			}
	...
	public function collect_qrcode(){
		$url='http://'.$_SERVER['SERVER_NAME']."/youbangqy/web/collect.php?id=".$_REQUEST[id];
		$this->url=$url;
		$this->display();
	}
